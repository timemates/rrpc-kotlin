            ...
            .initializer(
                buildCodeBlock {
                    ...
                    service.rpcs.forEach { rpc ->
                        ...
                        add(
                            "options = %L",
                            RawOptionsCodeGeneration.generate(rpc.options, schema, Options.METHOD_OPTIONS)
                        )
                        newline(before = ",")
                        unindent()
                        add("),")
                    ...
                    add(
                        "options = %L",
                        RawOptionsCodeGeneration.generate(service.options, schema, Options.SERVICE_OPTIONS)
                    )
                    newline()
                    unindent()
                    add(")")
