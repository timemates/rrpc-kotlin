...
internal class MessageValuesSerializer(
    private val type: RMType.Message,
                    ...
                    when (field.typeUrl) {
                        ...
                        RMDeclarationUrl.ACK, RMDeclarationUrl.EMPTY -> ProtoEmpty.serializer()
                            ...
                            return compositeEncoder.encodeNullableSerializableElement(
                                ...
                                serializer,
                                when (type) {
                                    is RMType.Enclosing -> Unit
                                    is RMType.Enum -> value.getInt(field.tag)
                                    is RMType.Message -> value.getMessage(field.tag)
                                },
                            )
                        }
                    }
    ...
    return when (url) {
        ...
        RMDeclarationUrl.ACK, RMDeclarationUrl.EMPTY -> ProtoEmpty.serializer()
            ...
            when (val type = resolver.resolveType(url)!!) {
                is RMType.Enum -> Int.serializer()
                is RMType.Enclosing -> Unit.serializer()
                is RMType.Message -> factory.getMessageSerializer(type)
            }
        }
    } as KSerializer<Any>
}