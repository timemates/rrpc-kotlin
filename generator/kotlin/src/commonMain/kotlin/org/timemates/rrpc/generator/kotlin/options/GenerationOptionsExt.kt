...
public val GenerationOptions.Companion.METADATA_GENERATION: SingleGenerationOption<MetadataGenerationType> by lazy {
    GenerationOption.single(
        name = "kotlin_metadata_generation",
        description = "Specifies whether metadata should be generated. Possible options: global, scoped, disabled (default).",
        valueKind = OptionTypeKind.Choice(enumEntries<MetadataGenerationType>().map { it.name.lowercase() }),
        constructor = { MetadataGenerationType.valueOf(it.uppercase()) }
    )
}
...
public val GenerationOptions.Companion.METADATA_SCOPE_NAME: SingleGenerationOption<String> by lazy {
    GenerationOption.single(
        ...
        description = "Specifies the scope name for metadata generation.",
        valueKind = OptionTypeKind.Text,
        constructor = { it }
    )
}